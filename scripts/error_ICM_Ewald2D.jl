...
begin
    ...
    # data_file = CSV.write("data/error_ICM_Ewald2D.csv", DataFrame(i = Int64[], H = Float64[], M = Float64[], γu = Float64[], γd = Float64[], energy = Float64[], error_r = Float64[], error_a = Float64[]))
    for i in 1:10
        for H in [10.0, 5.0, 1.0, 0.5, 0.1]
            jld_path = "reference_results/Lx_10.0_Ly_10.0_H_$(H)_n_39_seed_$(i).jld2"
            # for (γu, γd) in [(0.6, 0.6), (0.95, 0.95), (1.0, 1.0)]
            for (γu, γd) in [(0.3, 0.3)]
                for M in 1:2:30
                    s = 6.0
                    ...
                    CSV.write("data/error_ICM_Ewald2D.csv", DataFrame(i = i, H = H, M = M, γu = γu, γd = γd, energy = energy_ewald, error_r = error_r, error_a = error_a), append = true)
                end
            end
        end
    end
