...
data_file = "data/error_ICM_EwaldELC_force.csv"
...
begin
    ...
    save("figs/error_icm_pad_gamma_1_force.pdf", f, px_per_unit = 2)
    save("figs/error_icm_pad_gamma_1_force.png", f, px_per_unit = 2)
    f
end
...
begin
    ...
    M = 49
    for (r, c, ls, ms) in [(1, colors[1], linestyle[1], markerstyle[1]), (3, colors[2], linestyle[2], markerstyle[2]), (5, colors[3], linestyle[3], markerstyle[3])]
        scatter!(ax2, df[df.γu .== gamma .&& df.γd .== gamma .&& df.H .== H .&& df.r .== r, :M][1:sp2:11], df[df.γu .== gamma .&& df.γd .== gamma .&& df.H .== H .&& df.r .== r, :error_r][1:sp2:11], label = "$r", marker = ms, markersize = markersize, color = c, strokecolor = strokecolor, strokewidth = strokewidth)
    end
    axislegend(ax2, L"$P$", position = :lb)
    xlims!(ax2, -2.5, 52.5)
    ylims!(ax2, 1e-15, 1e2)
    ...
    save("figs/error_icm_pad_gamma_0.6_force.pdf", f, px_per_unit = 2)
    save("figs/error_icm_pad_gamma_0.6_force.png", f, px_per_unit = 2)
    f
end
...
begin
    ...
    xlims!(ax1, -2, 102)
    ylims!(ax1, 1e-10, 1e3)
    text!(ax1, 100, 200, text = "(b)", fontsize = 30, align = (:right, :center))
    ...
    save("figs/icm_elc_error_force.pdf", f, px_per_unit = 2)
    save("figs/icm_elc_error_force.png", f, px_per_unit = 2)
    f
end